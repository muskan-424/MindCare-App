/**
 * Patch profile dialog keys, safety, and institution sections.
 */
import fs from 'fs';
import path from 'path';
import { fileURLToPath } from 'url';
import translations from '../src/localization/translations.js';
import { PROFILE_SAFETY_INSTITUTION_BY_LANG } from './profile-safety-institution-locale-patches.mjs';
...
for (const lang of LANGS) {
  const patch = PROFILE_SAFETY_INSTITUTION_BY_LANG[lang];
  if (!patch) continue;
...
console.log(`patched profile/safety/institution for ${Object.keys(PROFILE_SAFETY_INSTITUTION_BY_LANG).length} languages`);
